...
WEIGHT_QUANTIZER = "quantized_bits(10,2,1)"
PHI_RELU1_QUANTIZER = "quantized_relu(10,3)"
PHI_RELU2_QUANTIZER = "quantized_relu(10,4)"
RHO_RELU1_QUANTIZER = "quantized_relu(10,2)"
...
class BestPuppiDataset(Dataset):
    def __init__(self, pattern: str, max_files=10):
        ...
        for f in self.files:
            ...
            if isinstance(payload, dict) and "graphs" in payload:
                ...
                if self.metadata is None:
                    ...
                    raise RuntimeError(f"Feature-name mismatch in {f}\nexisting: {self.feature_names}\nnew     : {feature_names}")
            else:
                graphs = payload
                metadata = {}
                feature_names = None
        ...
        if self.feature_names is None:
            ...
            self.metadata = {"feature_names": self.feature_names, "note": "Inferred feature names from old-format dataset."}
    ...
    def __getitem__(self, idx):
        ...
        return {"x": x_fixed, "mask": mask, "y": data.y, "event_idx": data.event_idx, "center_idx": data.center_idx}
...
def build_feature_indices(dataset_feature_names, input_features):
    ...
    if missing:
        raise RuntimeError(f"Requested input features are missing from dataset metadata:\nmissing = {missing}\navailable = {dataset_feature_names}")
...
def build_model(nmax=NMAX, in_dim=len(INPUT_FEATURES), hidden_dim=16):
    ...
    h = QDense(hidden_dim, kernel_quantizer=WEIGHT_QUANTIZER, bias_quantizer=WEIGHT_QUANTIZER, name="phi_dense1", **pytorch_dense_kwargs(in_dim))(x_input)
    h = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name="phi_bn1")(h, mask=mask_input)
    h = QActivation(PHI_RELU1_QUANTIZER, name="phi_relu1")(h)

    h = QDense(hidden_dim, kernel_quantizer=WEIGHT_QUANTIZER, bias_quantizer=WEIGHT_QUANTIZER, name="phi_dense2", **pytorch_dense_kwargs(hidden_dim))(h)
    h = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name="phi_bn2")(h, mask=mask_input)
    h = QActivation(PHI_RELU2_QUANTIZER, name="phi_relu2")(h)

    h_mean = layers.GlobalAveragePooling1D(name="masked_mean")(h, mask=mask_input)

    out = QDense(hidden_dim, kernel_quantizer=WEIGHT_QUANTIZER, bias_quantizer=WEIGHT_QUANTIZER, name="rho_dense1", **pytorch_dense_kwargs(hidden_dim))(h_mean)
    out = QActivation(RHO_RELU1_QUANTIZER, name="rho_relu1")(out)
    raw = QDense(1, kernel_quantizer=WEIGHT_QUANTIZER, bias_quantizer=WEIGHT_QUANTIZER, name="rho_output", **pytorch_dense_kwargs(hidden_dim))(out)
    pred = layers.Activation("softplus", name="puppi_weight")(raw)

    return keras.Model(inputs={"x": x_input, "mask": mask_input}, outputs=pred, name="PuppiDeepSetMeanOnlyQKeras")


def puppi_loss(pred, target, loss_type="weighted_huber", alpha=1.0, eps=1e-3, zero_thr=0.05, pos_weight=9.0, pos_weight_threshold=0.05):
    ...
    if loss_type in ("huber", "weighted_huber"):
        ...
        if loss_type == "weighted_huber":
            weight = tf.where(target > pos_weight_threshold, tf.cast(pos_weight, pred.dtype), tf.cast(1.0, pred.dtype))
    ...
    if alpha > 0.0:
        ...
        def logratio_term():
            ...
            t = t = tf.maximum(target_pos, tf.cast(eps, target.dtype))
            log_ratio = tf.math.log(p) - tf.math.log(t)
            return tf.reduce_mean(tf.square(log_ratio))

        loss_logratio = tf.cond(tf.size(pred_pos) > 0, logratio_term, lambda: tf.cast(0.0, pred.dtype))
...
def prepare_batch(data, feature_idx):
    ...
    return tf.convert_to_tensor(x), tf.convert_to_tensor(mask), tf.convert_to_tensor(target)
...
def train_epoch(model, loader, optimizer, args, feature_idx):
    ...
    for data in tqdm(loader, desc="Train", leave=False):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = puppi_loss(pred, target, loss_type=args.loss_type, alpha=args.penalty_alpha, eps=args.penalty_eps, zero_thr=args.penalty_zero_thr, pos_weight=args.pos_weight, pos_weight_threshold=args.pos_weight_threshold)
...
def main():
    ...
    train_loader = DataLoader(torch.utils.data.Subset(dataset, train_idx), batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=False)
    val_loader = DataLoader(torch.utils.data.Subset(dataset, val_idx), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=False)
    test_loader = DataLoader(torch.utils.data.Subset(dataset, test_idx), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=False)

    model = build_model(nmax=NMAX, in_dim=len(INPUT_FEATURES), hidden_dim=args.hidden_dim)
    model.summary()

    n_trainable = int(np.sum([np.prod(v.shape) for v in model.trainable_variables]))
    print(f"Trainable parameters: {n_trainable:,}")

    optimizer = keras.optimizers.AdamW(learning_rate=args.lr, weight_decay=0.01, beta_1=0.9, beta_2=0.999, epsilon=1e-8)
    ...
    config["framework"] = "tensorflow_keras_qkeras"
    config["model"] = "DeepSet_mean_only_QKeras"
    ...
    config["weight_quantizer"] = WEIGHT_QUANTIZER
    config["phi_relu1_quantizer"] = PHI_RELU1_QUANTIZER
    config["phi_relu2_quantizer"] = PHI_RELU2_QUANTIZER
    config["rho_relu1_quantizer"] = RHO_RELU1_QUANTIZER
    ...
    print("Start QKeras training!")

    for epoch in range(1, args.epochs + 1):
        train_loss = train_epoch(model, train_loader, optimizer, args, feature_idx)
        val_out = evaluate(model, val_loader, args, feature_idx, input_feature_name_to_idx)
        val_loss = val_out["loss"]

        diag = compute_diagnostic_metrics(val_out["pred"], val_out["target"], eps=args.penalty_eps, zero_thr=args.penalty_zero_thr, pred_thr=args.pos_weight_threshold)
        ...
        print(
            ...
            f"val_mse={diag['mse']:.5f} | val_huber={diag['huber']:.5f} | "
        ...
        np.savez(os.path.join(run_dir, "loss_history.npz"), **{k: np.asarray(v) for k, v in history.items()})
        model.save_weights(os.path.join(run_dir, f"model_epoch{epoch}.weights.h5"))

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            best_epoch = epoch
            model.save_weights(os.path.join(run_dir, "model_best.weights.h5"))
            np.savez(os.path.join(run_dir, "val_outputs_best.npz"), **val_out)

    print(f"Best epoch: {best_epoch} with val_loss={best_val_loss:.6f}")

    model.load_weights(os.path.join(run_dir, "model_best.weights.h5"))

    test_out = evaluate(model, test_loader, args, feature_idx, input_feature_name_to_idx)
    test_diag = compute_diagnostic_metrics(test_out["pred"], test_out["target"], eps=args.penalty_eps, zero_thr=args.penalty_zero_thr, pred_thr=args.pos_weight_threshold)
